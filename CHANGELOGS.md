...
* UPDATED: Speed Limit Control (SLC)
  * NEW❗: Speed Limit Engage Mode
    * Select the desired mode to set the cruising speed to the speed limit
      * Warning Only: Provide a warning when the speed limit is exceeded
      * Auto: Adjust the cruising speed to the posted speed automatically, plus or minus the offset you have chosen
      * User Confirm:
        * Adjust the cruising speed to the posted speed, plus or minus the offset, with one click of the cruise control up and down toggle switch
        * sunnypilot will notify you if your current speed is different from the posted speed, plus or minus the offset that you have chosen
        * You must acknowledge and initiate the change in vehicle speed by pressing plus or minus on the cruise control speed adjustment switch
    * Supported platforms
      * openpilot Longitudinal Control available cars (Exclude Toyota/Lexus, explained below)
      * Custom Stock Longitudinal Control available cars
    * Unsupported platforms
      * Toyota/Lexus - most platforms do not allow us to control the PCM's set speed, requires testers to verify
